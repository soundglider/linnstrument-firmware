...
byte function_color(uint8_t function_id) {
  switch (function_id) {
    case FN_T:  return COLOR_CYAN;     // tonic / mediant / submediant: teal-ish
    case FN_PD: return COLOR_BLUE;     // predominant
    case FN_D:  return COLOR_RED;      // dominant / leading-tone
    case FN_X:  return COLOR_MAGENTA;  // chromatic / modal mixture
...
void chord_grid_repaint() {
  ...
  for (uint8_t row = 0; row < 8; ++row) {
    for (uint8_t local_col = 0; local_col < CHORD_GRID_COLS; ++local_col) {
      const ChordTemplate* tpl = chord_template_for_cell(row, local_col);
      byte sensor_col = local_col + 1;
      if (tpl == nullptr) {
        setLed(sensor_col, row, COLOR_OFF, cellOff, LED_LAYER_MAIN);
      } else {
        setLed(sensor_col, row, function_color(tpl->function), cellOn, LED_LAYER_MAIN);
      }
    }
  }
}
